...
impl Tool for Cargo {
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
            ...
            .map(|c| c.inner)
        {
            ...
            // Add doc links - docs.rs is the standard for autogenerated
            // documentation for any rust crate, so we always provide it
            lines.push(String::new());
            ...
            if let Some(docs) = docs {
                if docs.contains("docs.rs") {
                    ...
                    lines.push(format!(
                        "- [Documentation]({docs}) ([docs.rs]({DOCS_RS}/{}))",
                        crate_data.name
                    ));
                }
            } else {
                lines.push(format!("- [Documentation]({DOCS_RS}/{})", crate_data.name))
            }
