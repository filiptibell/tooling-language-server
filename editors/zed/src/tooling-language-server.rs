...
struct ToolingLanguageServerExtension {
    cached_binary_path: Option<String>,
}

impl ToolingLanguageServerExtension {
    fn language_server_binary_path(
        &mut self,
        language_server_id: &LanguageServerId,
        worktree: &zed::Worktree,
    ) -> Result<String> {
        if let Some(path) = worktree.which("tooling-language-server") {
            return Ok(path);
        }
        ...
        let version_dir_name = format!("tooling-language-server-{}", release.version);
        let version_dir = format!("{BINARY_DIR}/{version_dir_name}");
        let binary_path = format!(
            "{version_dir}/tooling-language-server{extension}",
...
impl zed::Extension for ToolingLanguageServerExtension {
    fn new() -> Self {
        Self {
    ...
    fn run_slash_command(
        ...
        match command.name.as_str() {
            "tooling-language-server-set-github-personal-access-token" => {
                github_auth::run_set_personal_access_token(command, args)
            }
            "tooling-language-server-remove-github-personal-access-token" => {
...
zed::register_extension!(ToolingLanguageServerExtension);
